...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self.statusLabel setText:@"Place the finger of the hand to the camera so that it covers the camera and the flash."];
}
...
- (void)animate
{
    [UIView animateWithDuration:2.8 delay:0.2 options: UIViewAnimationOptionAutoreverse | UIViewAnimationOptionRepeat animations:^{
        [self.imageView setFrame:CGRectMake(self.view.center.x, self.view.center.y-100, 140, 140)];
...
- (void)viewDidLoad
{
    ...
    [self.statusLabel setNumberOfLines:3];
    self.statusLabel.font = [UIFont systemFontOfSize:20];
    [self.view addSubview:self.statusLabel];
    
    [self.statusLabel.leftAnchor constraintEqualToAnchor:self.view.leftAnchor constant:0].active = true;
    [self.statusLabel.rightAnchor constraintEqualToAnchor:self.view.rightAnchor constant:0].active = true;
    [self.statusLabel.bottomAnchor constraintEqualToAnchor:self.view.centerYAnchor constant:0].active = true;
    
    
    
    self.bpmLabel = [[UILabel alloc] init];
    self.bpmLabel.translatesAutoresizingMaskIntoConstraints = NO;
    [self.bpmLabel setTextAlignment:NSTextAlignmentCenter];
    [self.bpmLabel setTextColor:[UIColor greenColor]];
    [self.bpmLabel setNumberOfLines:3];
    
    self.bpmLabel.font = [UIFont systemFontOfSize:HRKLabelFontSize];
    [self.view addSubview:self.bpmLabel];
    [self.bpmLabel.topAnchor constraintEqualToAnchor:self.statusLabel.bottomAnchor constant:5].active = true;
    [self.bpmLabel.rightAnchor constraintEqualToAnchor:self.view.rightAnchor constant:0].active = true;
    [self.bpmLabel.leftAnchor constraintEqualToAnchor:self.view.leftAnchor constant:0].active = true;
    
    
    self.cancelButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [self.cancelButton setImage:[UIImage imageNamed:@"dismiss"] forState:UIControlStateNormal];
    [self.cancelButton setTintColor:[UIColor greenColor]];
    self.cancelButton.translatesAutoresizingMaskIntoConstraints = NO;
    [self.cancelButton addTarget:self action:@selector(cancelButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.cancelButton];
    [self.cancelButton.topAnchor constraintEqualToAnchor:self.view.topAnchor constant:8].active = true;
    [self.cancelButton.rightAnchor constraintEqualToAnchor:self.view.rightAnchor constant:-8].active = true;
    [self.cancelButton.heightAnchor constraintEqualToConstant:40].active = true;
    [self.cancelButton.widthAnchor constraintEqualToConstant:40].active = true;
    
    
    CGRect frame = CGRectMake(self.view.center.x, self.view.center.y, 120, 120);
    self.imageView = [[UIImageView alloc] initWithFrame:frame];
    self.imageView.center = CGPointMake(self.view.center.x, self.view.center.y-100);
    [self.imageView setImage:[UIImage imageNamed:@"cardiogram"]];
    [self.imageView setTintColor:[UIColor greenColor]];
    ...
    dispatch_async(algorithmQueue, ^{
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            if (self.algorithm.shouldShowLatestResult) {
                ...
                if (timeTillResult >= 0) {
                    self.statusLabel.text = [NSString stringWithFormat:@"Left to wait before result: %.01fs" , timeTillResult];
                }
            }
            ...
            if (self.algorithm.shouldShowLatestResult) {
                ...
                self.bpmLabel.text = @"Waiting for BPM results. Try not to move your finger";
            }
